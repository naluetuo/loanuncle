...
import com.loanuncle.gm.loanuncle.util.DisplayUtils;
...
public class DatePickerView extends View {
    ...
    private void init() {
        ...
        //被选中的数字颜色
        mPaint.setColor(ContextCompat.getColor(context, R.color.login_button));
        //第二个paint
        nPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        nPaint.setStyle(Style.FILL);
        nPaint.setTextAlign(Align.CENTER);
        //未选中的数字颜色
        nPaint.setColor(ContextCompat.getColor(context, R.color.login_button));
        //画线的paint
    ...
    private void drawData(Canvas canvas) {
        ...
        float line_y1 = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0)+DisplayUtils.dp2px(context,5));
        float line_y2 = (float) (y + (fmi.bottom / 2.0 + fmi.top / 2.0)-DisplayUtils.dp2px(context,5));

        if (mDataList.size() > 0){
            canvas.drawText(mDataList.get(mCurrentSelected), x, baseline, mPaint);
        }
        canvas.drawLine(0,line_y1,mViewWidth,line_y1,linePaint);
        canvas.drawLine(0,line_y2,mViewWidth,line_y2,linePaint);
        // 绘制上方data
    ...
    public void setTextSize(int sp){
        this.textSize = DisplayUtils.sp2px(context,sp);
    }
